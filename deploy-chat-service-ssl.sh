...
# BIDR Chat Service HTTPS Deployment Script
# This script deploys the chat service behind Azure Application Gateway with SSL termination

set -e

echo "🚀 BIDR Chat Service HTTPS Deployment"
echo "====================================="

# Configuration
RESOURCE_GROUP="bidr-simple-rg"
CONTAINER_NAME="bidr-auth-service"  # Currently using auth service as backend
BACKEND_POOL="chatServiceBackendPool"
GATEWAY_NAME="bidr-appgw"
DOMAIN="chat-service.bidr.co.za"
HTTP_SETTINGS="chatServiceBackendHttpSettings"
...
CERT_STATUS=$(az network application-gateway ssl-cert show \
  --resource-group $RESOURCE_GROUP \
  --gateway-name $GATEWAY_NAME \
  --name chat-service-bidr-co-za-cert \
...
echo "===================="
echo "✅ Chat service is accessible via HTTPS"
...
echo "   Deploy a dedicated chat service container when ready."
echo ""
echo "✅ Chat service HTTPS deployment completed successfully!"
