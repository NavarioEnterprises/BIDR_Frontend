...
# BIDR Notifications Service - Azure Container Instances Deployment Script
# This script builds and deploys the notifications service to Azure Container Instances
...
CONFIG_FILE="$SCRIPT_DIR/notifications-service-config.env"
...
echo "========================================"
echo "🚀 BIDR Notifications Service Deployment"
echo "========================================"
...
# Verify we're in the notifications service directory
if [ ! -f "$SERVICE_DIR/manage.py" ]; then
    log_error "manage.py not found in $SERVICE_DIR. Are you in the correct directory?"
    exit 1
fi

log_success "Found notifications service directory: $SERVICE_DIR"
...
# Step 8: Test notifications service
log_step "Step 8: Testing notifications service..."
if curl -f http://$CONTAINER_IP:$SERVICE_PORT/health/ &>/dev/null; then
    log_success "Health endpoint is responding"
else
    log_warning "Health endpoint not responding, but service may still be starting"
fi
...
echo "• SMS API: $PRODUCTION_URL/api/v1/sms/"
...
echo "3. Configure SMS and email provider credentials if needed"
echo "4. Test the service via the production domain"

echo ""
echo "🎊 Your notifications service is now deployed and ready for gateway configuration!"
echo "========================================"
