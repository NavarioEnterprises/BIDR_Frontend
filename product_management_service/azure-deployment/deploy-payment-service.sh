...
SERVICE_DIR="$(dirname "$SCRIPT_DIR")/payment_service"

# Check if payment service directory exists
if [ ! -d "$SERVICE_DIR" ]; then
    print_error "Payment service directory not found at: $SERVICE_DIR"
    print_error "Please make sure you're running this from the correct location."
    exit 1
fi

print_status "Found payment service directory: $SERVICE_DIR"
