...
class BIDRDeploymentManager:
    def __init__(self):
        ...
        self.services = {
            ...
            "2": {
                ...
                "icon": "🔐",
                "script": "deploy-auth-service-with-gateway.sh",
                "update_script": "deploy-auth-service-with-gateway.sh",
                "https_enabled": True,
                "public_url": "https://api.bidr.co.za"
            },
            "3": {
                "name": "Chat Service",
                "service_key": "chat",
    ...
    def print_services_menu(self):
        ...
        for key, service in self.services.items():
            ...
            https_badge = f" {Colors.GREEN}🔒HTTPS{Colors.ENDC}" if service.get('https_enabled', False) else ""
            print(f"{Colors.YELLOW}{key}. {service['icon']} {service['name']} {status_icon}{https_badge}{Colors.ENDC}")
            suggested_path = self.base_dir / service['directory']
            print(f"   {Colors.BLUE}Path: {suggested_path}{Colors.ENDC}")
            port_info = f"Port: {service['port']} | Container: {service['container_name']}"
            if service.get('public_url'):
                port_info += f" | URL: {service['public_url']}"
            print(f"   {Colors.GREEN}{port_info}{Colors.ENDC}")
            print()
