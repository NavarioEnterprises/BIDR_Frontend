...
# from authentication_service.user.models import AppUser
...
class ProductRequestAnalytics(BaseModel):
    """
    Analytics data for product requests.
    """
    ...
    search_appearances = models.IntegerField(
        default=0,
        help_text="Times request appeared in search results"
    )
    search_clicks = models.IntegerField(
        default=0,
        help_text="Times request was clicked from search results"
    )
    
    # Response metrics
    quote_responses = models.IntegerField(
        default=0,
        help_text="Number of quote responses received"
    )
    messages_sent = models.IntegerField(
        default=0,
        help_text="Number of messages sent to requestor"
    )
    watchlist_additions = models.IntegerField(
        default=0,
        help_text="Number of times added to watchlist"
    )
    ...
    average_response_time = models.DurationField(
        null=True,
        blank=True,
        help_text="Average response time for quotes"
    )
    supplier_interest_score = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        default=Decimal('0.00'),
        help_text="Score based on supplier engagement (0-100)"
    )
    ...
    class Meta:
        ...
        ordering = ['-date', '-created_at']
        indexes = [
            models.Index(fields=['request', 'date']),
            models.Index(fields=['date', 'timeframe']),
            models.Index(fields=['supplier_interest_score', 'date']),
        ]
    
    def __str__(self):
        return f"Request {self.request.request_id} analytics for {self.date} ({self.timeframe})"
    ...
    def response_rate(self):
        """Calculate response rate based on views to quotes."""
        if self.views == 0:
            return Decimal('0.00')
        return round(Decimal(self.quote_responses) / Decimal(self.views) * 100, 2)
...
class CategoryAnalytics(BaseModel):
    ...
    total_requests = models.IntegerField(
        default=0,
        help_text="Total number of product requests in category"
    )
    active_requests = models.IntegerField(
        default=0,
        help_text="Number of active product requests"
    )
    new_requests = models.IntegerField(
        default=0,
        help_text="New product requests added in this period"
    )
    closed_requests = models.IntegerField(
        default=0,
        help_text="Number of closed requests in this period"
    )
