...
class BuyerMobileDashboard extends StatefulWidget {
  @override
  _BuyerMobileDashboardState createState() => _BuyerMobileDashboardState();
}

bool isFromHomepage = false;

class _BuyerMobileDashboardState extends State<BuyerMobileDashboard> {
  ...
  int get _totalPages {
    return (_productRequests.length / _itemsPerPage)
        .ceil()
  ...
  Widget _buildContent(
    ...
    switch (index) {
      ...
      case 1:
        return ShareWidgetMobile();
      case 2:
        return TransactionMobileDashboard(key: _transactionKey);
      case 3:
  ...
  Widget _buildMobileRequestsGrid(
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            ResponsiveGap(type: SpacingType.small),
            ResponsiveText(
              text: 'Create your first product request to get started',
              type: TextType.normal,
    ...
    final paginatedRequests = filteredRequests.sublist(
      startIndex.clamp(0, filteredRequests.length),
      endIndex,
    );
    final totalPages = (filteredRequests.length / _itemsPerPage)
        .ceil()
        .clamp(1, double.infinity)
        .toInt();
          ...
          children: [
            if (!isNext) ...[
              Icon(
                icon,
                size: 16,
                color: isEnabled ? Constants.ctaColorLight : Colors.grey,
              ),
            ...
            if (bidsToShow.isNotEmpty) ...[
              ...bidsToShow
                  .map((bid) {
                    ...
                    Map<int, int> sellerIndexMap = _createSellerIndexMapping(
                      bidsToShow,
                    );
                    int sellerIndex = sellerIndexMap[sellerId] ?? 1;

                    return _buildModernSellerBid(bid, request, sellerIndex);
                  })
                  .take(1),
    ...
    try {
      ...
      if (request is AutoSparesRequest) {
        ...
        if (request.rimTyre?.productDetails != null) {
          final tyreTypeRaw =
              request.rimTyre.productDetails.tyreType ?? "Tyres";
        ...
        case "TYRES_RIMS":
          ...
          if (request.vehicleTyresRimsData != null ||
            ...
            final tyreTypeRaw =
                ...
                "Tyres";
            final tyreType = tyreTypeRaw == "Both"
                ? "Tyres & Rims"
                : tyreTypeRaw;
            final tyreWidth =
            ...
            if (tyreWidth != 0 &&
              ...
              return "$tyreType, $tyreWidth/$sidewall" +
                  "R$rimDiameter, $brand";
            }
          }
  ...
  String _getLowestBidText(dynamic request) {
    try {
      ...
      offers.sort((a, b) {
        final priceA = double.tryParse(a?.unitPrice?.toString() ?? '0') ?? 0;
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: Text(
                'Offers for Request #${_getRequestId(request)}',
                style: GoogleFonts.manrope(
                  fontSize: 18,
                  fontWeight: FontWeight.bold,
                ),
        ...
        child: Column(
          children: [
            ...
            Padding(
              padding: EdgeInsets.all(16),
              child: Text(
                'Request #${_getRequestId(request)} Details',
                style: GoogleFonts.manrope(
                  fontSize: 18,
                  fontWeight: FontWeight.bold,
                ),
  ...
  String _getRequestDisplayTitle(dynamic request) {
    try {
      ...
      print('Error getting request title: $e');
    ...
    try {
      ...
      switch (request.category) {
        ...
        case "Consumer Electronics":
          ...
          dynamic electronicsData = electronicsRequest.consumerElectronics;
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => ConsumerElectronicsDetailScreen(
                ...
                consumerElectronics: electronicsData,
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => PaymentScreen(
          seller: seller,
          request: request,
          onPaymentComplete: _processPaymentAndCreateOrder,
        ),
      ),
    );
  }
  ...
  Future<void> _fetchProductRequests({bool showLoading = true}) async {
    print(
      '=== DEBUG: _fetchProductRequests called, showLoading: $showLoading ===',
    );
    if (!mounted) {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        if (mounted) {
          print('=== DEBUG: Setting error state due to API error ===');
          setState(() {
          ...
          print(
            '=== DEBUG: Widget not mounted, skipping error state update ===',
          );
        }
      }
    } catch (e) {
      ...
      if (mounted) {
        print('=== DEBUG: Setting error state due to exception ===');
        setState(() {
          _error = 'Failed to load requests';
          _isLoading = false;
          _isInitialLoad = false;
        ...
        print('=== DEBUG: Widget not mounted, skipping error state update ===');
      }
    }
  }
  ...
  RimTyreRequest _createRimTyreRequest(ProductRequestItem item) {
    ...
    // Extract data from vehicle_tyres_rims_data or product_specifications
    final tyresRimsData = item.vehicleTyresRimsData ?? {};
    final specs = item.productSpecifications ?? {};

    print('🔍 MAPPING: tyresRimsData = $tyresRimsData');
    print('🔍 MAPPING: specs = $specs');
    print('🔍 MAPPING: productImages = ${item.productImages}');
  ...
  ConsumerElectronicsRequest _createElectronicsRequest(
    ...
    // Extract data from consumer_electronics_data or product_specifications
    final electronicsData = item.consumerElectronicsData ?? {};
    final specs = item.productSpecifications ?? {};

    print('🔍 ELECTRONICS MAPPING: electronicsData = $electronicsData');
    print('🔍 ELECTRONICS MAPPING: specs = $specs');
    print('🔍 ELECTRONICS MAPPING: productImages = ${item.productImages}');
    ...
    final budgetTimeline = BudgetTimeline(
      minPrice: specs['min_price'] != null
          ? double.tryParse(specs['min_price'].toString())
          : null,
      maxPrice: specs['max_price'] != null
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        ...
        if (mounted) {
          ...
          Future.delayed(Duration(seconds: 1), () {
            if (mounted) {
              print('=== DEBUG: Refreshing product requests after delay ===');
              _fetchProductRequests(showLoading: false);
            } else {
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      Navigator.of(context).pop();
                      print('=== DEBUG: Success dialog closed ===');
                    },
                    style: ElevatedButton.styleFrom(
                      backgroundColor: Colors.green[600],
                      foregroundColor: Colors.white,
  ...
  try {
    ...
    switch (request.category) {
      ...
      case "TYRES_RIMS":
        if (request.tyresRimsSummary != null &&
            request.tyresRimsSummary.isNotEmpty) {
          return request.tyresRimsSummary;
        }
        return "Tyre/Rim Request";
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      SizedBox(height: 8),
                                      Text(
                                        _getRequestDescription(widget.request),
                                        style: GoogleFonts.manrope(
                                          fontSize: 13,
                                          color: Colors.black87,
                                          fontWeight: FontWeight.w600,
                ...
                return Container(
                  ...
                  child: GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return GestureDetector(
                        onTap: () =>
                            _showImagePreview(context, productImages, index),
                        child: Container(
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(8),
                            border: Border.all(color: Color(0xFFE0E0E0)),
...
class _ConsumerElectronicsDetailScreenState
                        ...
                        child: ClipRRect(
                          ...
                          child: Image.network(
                            (images[index].startsWith('http')
                                ? images[index]
                                : '${GlobalVariables.productsServiceUrl}${images[index].startsWith('/') ? images[index].substring(1) : images[index]}'),
                            fit: BoxFit.contain,
                            loadingBuilder: (context, child, loadingProgress) {
                              if (loadingProgress == null) return child;
                              return Container(
                              ...
                              Expanded(
                                child: _buildDetailCard(
                                  ...
                                  [
                                    ...
                                    /* _buildDetailItem(
                                      "Max Distance You Want to Travel (km)",
                                      widget.consumerElectronics.productDetails.maxDistanceKm
                                          .toString()??"-",
                                    ),*/
                                    _buildDetailItem(
                                      "Quantity Needed",
            ...
            if (imageList != null && imageList.isNotEmpty) ...[
              Container(
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return GestureDetector(
                      onTap: () => _showImagePreview(context, imageList, index),
                      child: Container(
                        decoration: BoxDecoration(
                          borderRadius: BorderRadius.circular(8),
                          border: Border.all(color: Color(0xFFE0E0E0)),
                        ...
                        child: ClipRRect(
                          ...
                          child: Image.network(
                            (imageList[index].startsWith('http')
                                ? imageList[index]
                                : '${GlobalVariables.productsServiceUrl}${imageList[index].startsWith('/') ? imageList[index].substring(1) : imageList[index]}'),
                            fit: BoxFit.cover,
                            loadingBuilder: (context, child, loadingProgress) {
                              if (loadingProgress == null) return child;
                              return Container(
  ...
  Future<void> _cancelRequest() async {
    ...
    print('=== DEBUG: Cancel Request from ConsumerElectronicsDetailScreen ===');
    print('Request type: ${widget.request.runtimeType}');
    print('Request ID: $requestId');
...
class _RimTyreDetailScreenState extends State<RimTyreDetailScreen> {
                              ...
                              Expanded(
                                child: _buildDetailCard(
                                  ...
                                  [
                                    ...
                                    _buildDetailItem(
                                      "Tyre Size",
                                      "${widget.rimTyre.productDetails.tyreWidthMm ?? 0}/${widget.rimTyre.productDetails.sidewallProfile ?? '0'}R${widget.rimTyre.productDetails.wheelRimDiameterInches ?? '0'}",
                                    ),
                                    /*_buildDetailItem(
                                      "Max Distance You Want to Travel (km)",
                                      widget.rimTyre.productDetails.maxDistanceKm
                                          .toString()??"-",
                                    ),*/
                                  ],
                                ),
                            ...
                            children: [
                              Expanded(
                                child: Builder(
                                  builder: (context) {
                                    print(
                                      '🔍 SERVICE DEBUG: Full rimTyre.moreFields object:',
                                    );
                                    print(
                                      '🔍 SERVICE DEBUG: ${widget.rimTyre.moreFields}',
                                    );
                                    print(
                                      '🔍 SERVICE DEBUG: rimTyre.moreFields.runtimeType = ${widget.rimTyre.moreFields.runtimeType}',
                                    );
                                    return _buildDetailCard(
                                      "Service Requirements1",
                                      Colors.orange,
                                      [
                                        _buildDetailItem("Fitment Required", () {
                                          final value = widget
                                              .rimTyre
                                              .moreFields
                                              .fitmentRequired;
                                          print(
                                            '🔍 SERVICE: fitmentRequired = "$value" (type: ${value.runtimeType})',
                                          );
                                        ...
                                        _buildDetailItem(
                                          ...
                                          () {
                                            final value = widget
                                                ...
                                                .balancingRequired;
                                            print(
                                              '🔍 SERVICE: balancingRequired = "$value" (type: ${value.runtimeType})',
                                            );
                                            return value ?? 'Not specified';
                                          }(),
                                        ),
                                        _buildDetailItem(
                                          ...
                                          () {
                                            final value = widget
                                                ...
                                                .tyreRotationRequired;
                                            print(
                                              '🔍 SERVICE: tyreRotationRequired = "$value" (type: ${value.runtimeType})',
                                            );
                                            return value ?? 'Not specified';
                                          }(),
                                        ),
                                        _buildDetailItem(
                                          ...
                                          isProductImages: true,
                                          imageList: () {
                                            final imageUrls = widget
                                                .rimTyre
                                                .moreFields
                                                .imageUrls;
                                            print(
                                              '🔍 SERVICE: imageUrls = $imageUrls (type: ${imageUrls.runtimeType})',
                                            );
                                            print(
                                              '🔍 SERVICE: imageUrls length = ${imageUrls?.length ?? 0}',
                                            );
                                            return imageUrls;
                                          }(),
                                        ),
  ...
  Future<void> _cancelRequest() async {
    ...
    print('=== DEBUG: Cancel Request from RimTyreDetailScreen ===');
    print('Request type: ${widget.request.runtimeType}');
    print('Request ID: $requestId');
...
// New Payment Screen Widget
class PaymentScreen extends StatelessWidget {
  final dynamic seller;
  final dynamic request;
  final Function(dynamic, dynamic) onPaymentComplete;
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  await onPaymentComplete(seller, request);
                },
                style: ElevatedButton.styleFrom(
  ...
  double _getBidAmount(dynamic seller) {
    try {
      if (seller is Map && seller.containsKey('bidAmount')) {
        final bidAmount = seller['bidAmount'];
        if (bidAmount is num) {
          return bidAmount.toDouble();
        } else if (bidAmount is String) {
          return double.tryParse(bidAmount) ?? 0.0;
        }
      }
      return 0.0;
    } catch (e) {
      print('Error getting bid amount: $e');
      return 0.0;
    }
  }
}
