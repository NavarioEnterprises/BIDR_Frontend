...
class _BuyerMobileDashboardState extends State<BuyerMobileDashboard> {
  ...
  Widget _buildMobileRequestsGrid(
    ...
    final totalPages = (filteredRequests.length / _itemsPerPage).ceil().clamp(1, double.infinity).toInt();
    ...
    try {
      ...
      switch (request.category) {
        ...
        case "TYRES_RIMS":
          ...
          if (request.vehicleTyresRimsData != null || request.productSpecifications != null) {
            final tyresData = request.vehicleTyresRimsData ?? request.productSpecifications ?? {};
            final tyreTypeRaw = tyresData['tyre_type']?.toString() ?? tyresData['tyreType']?.toString() ?? "Tyres";
            final tyreType = tyreTypeRaw == "Both" ? "Tyres & Rims" : tyreTypeRaw;
            final tyreWidth = tyresData['tyre_width_mm'] ?? tyresData['tyreWidthMm'] ?? 0;
            final sidewall = tyresData['sidewall_profile']?.toString() ?? tyresData['sidewallProfile']?.toString() ?? "";
            final rimDiameter = tyresData['wheel_rim_diameter_inches']?.toString() ?? tyresData['wheelRimDiameterInches']?.toString() ?? "";
            final brand = tyresData['preferred_brand']?.toString() ?? tyresData['preferredBrand']?.toString() ?? "Various Brands";
  ...
  void _navigateToDetailScreenMobile(dynamic request, int index) {
    try {
      ...
      switch (request.category) {
        ...
        case "ELECTRONICS":
        case "Consumer Electronics":
        // Convert ProductRequestItem to ConsumerElectronicsRequest if needed
          dynamic electronicsData;
          if (request.runtimeType.toString().contains('ProductRequestItem')) {
            final electronicsRequest = _createElectronicsRequest(request);
            electronicsData = electronicsRequest.consumerElectronics;
          } else {
            electronicsData = request.consumerElectronics;
          }
                          ...
                          child: Column(
                            ...
                            children: [
                              ...allBids.asMap().entries.map((entry) {
                                ...
                                int sellerId = _getSellerId(bid);
                                Map<int, int> sellerIndexMap = _createSellerIndexMapping(allBids);
                                int sellerIndex = sellerIndexMap[sellerId] ?? 1;
                                ...
                                return AnimatedContainer(
                                  ...
                                  child: Container(
                                    margin: EdgeInsets.only(
                                        bottom: index < allBids.length - 1 ? 16 : 0
                                    ),
...
class _RimTyreDetailScreenState extends State<RimTyreDetailScreen> {
  ...
  List<dynamic> _getSortedBids(dynamic request) {
    ...
    if (request?.sellerOffers != null) {
      bids = List.from(request.sellerOffers);
      // Sort by total amount (lowest first)
      bids.sort((a, b) {
        final aAmount = a?.totalAmount ?? 0.0;
        final bAmount = b?.totalAmount ?? 0.0;
