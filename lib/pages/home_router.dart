  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        // Debug logging
        print('HomeRouter Debug - isLoggedIn: $isLoggedIn');
        print('HomeRouter Debug - userRole: $userRole');
        print('HomeRouter Debug - loginData length: ${loginData?.length ?? 0}');
        print('HomeRouter Debug - Constants.currentUser: ${Constants.currentUser?.toJson()}');

        // If not logged in, show login prompt
        if (!isLoggedIn) {
          print('HomeRouter Debug - Showing login prompt (not logged in)');
        ...
        if (userRole == null || userRole.isEmpty) {
          if (loginData != null && loginData.isNotEmpty) {
            try {
              ...
              print('HomeRouter Debug - Routing by login data role: ${loginResponse.user.role.toLowerCase()}');
              switch (loginResponse.user.role.toLowerCase()) {
                case 'buyer':
                  print('HomeRouter Debug - Returning BuyerHomePage');
                  return BuyerHomePage();
                case 'seller':
                  print('HomeRouter Debug - Returning SellerDashboard');
                  return SellerDashboard();
                default:
                  print('HomeRouter Debug - Unknown role, showing login prompt');
                  return _buildLoginPrompt(context);
              }
            } catch (e) {
        ...
        print('HomeRouter Debug - Routing by userRole: ${userRole.toLowerCase()}');
        switch (userRole.toLowerCase()) {
          case 'buyer':
            print('HomeRouter Debug - Returning BuyerHomePage (userRole)');
            return BuyerHomePage();
          case 'seller':
            print('HomeRouter Debug - Returning SellerDashboard (userRole)');
            return SellerDashboard();
          default:
            print('HomeRouter Debug - Default case, showing login prompt');
            return _buildLoginPrompt(context);
        }
      },
    );
