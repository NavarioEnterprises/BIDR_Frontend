...
# Database
# Use PostgreSQL with credentials from Kubernetes secrets
...
if os.path.exists('/etc/secrets/db'):
    ...
    DATABASES = {
        'default': {
            ...
            'NAME': os.environ.get('DB_NAME', 'bidr_auth'),
    ...
    if not DATABASES['default']['ENGINE']:
        DATABASES = {
            'default': {
                ...
                'NAME': config('DATABASE_NAME', default='bidr_auth'),
...
LOGGING = {
    ...
    'handlers': {
        ...
        'file': {
            'level': 'INFO',
            'class': 'logging.StreamHandler',  # Use StreamHandler instead of FileHandler for containers
            'formatter': 'verbose',
        },
    },
    'loggers': {
