...
# BIDR payment Service Update Script
set -e
echo "🔄 Updating BIDR payment Service..."
...
payment_DIR="$(dirname "$SCRIPT_DIR")/payment_service"

if [ ! -d "$payment_DIR" ]; then
    echo "❌ payment service directory not found at: $payment_DIR"
    exit 1
fi

cd "$payment_DIR"
echo "Changed to payment service directory"
...
echo "Service URL: http://${NEW_FQDN}:${payment_8003}"
echo "🎯 Health Check: curl http://${NEW_FQDN}:${payment_8003}/health/"
