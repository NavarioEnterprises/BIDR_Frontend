...
# BIDR Payment Service Deployment Script
# This script builds and deploys the payment service to Azure Container Instances

set -e

echo "🚀 Deploying BIDR Payment Service..."
...
IMAGE_NAME="bidr-payment-service"
SERVICE_PORT=8003
...
# Get the payment service directory path
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PAYMENT_SERVICE_DIR="$(dirname "$SCRIPT_DIR")/payment_service"

# Check if payment service directory exists
if [ ! -d "$PAYMENT_SERVICE_DIR" ]; then
    print_error "Payment service directory not found at: $PAYMENT_SERVICE_DIR"
    print_error "Please make sure you're running this from the correct location."
    exit 1
fi

print_status "Found payment service directory: $PAYMENT_SERVICE_DIR"

# Check if Dockerfile exists
if [ ! -f "$PAYMENT_SERVICE_DIR/Dockerfile" ]; then
    print_error "Dockerfile not found in payment service directory."
    exit 1
fi

# Change to payment service directory
cd "$PAYMENT_SERVICE_DIR"
print_status "Changed to payment service directory"
...
print_success "Payment service image built and pushed"
...
print_success "Payment service deployed"
...
echo "========================================"
echo "Service Name: BIDR Payment Service"
...
print_status "Step 5: Testing payment service..."
